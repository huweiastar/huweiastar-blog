...
export default function MomentsPage() {
  ...
  function toggleDay(date: string) {
    setExpandedDays((p) => { const n = new Set(p); if (n.has(date)) n.delete(date); else n.add(date); return n; });
    setExpandedId(null);
  }
  ...
  if (loading) {
    return (
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-12">
        <PageHeader icon={MessageSquare} title="说说" subtitle="记录生活中的小确幸" />
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-12">
      ...
      {visibleGroups.map((group, groupIdx) => {
        ...
        const isStacked = group.moments.length > 1 && !onlyViewId && !dayExpanded;
        const canFan = group.moments.length > 1 && !onlyViewId;
        return (
        <motion.div key={group.date} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center gap-2 md:gap-3 mb-3 md:mb-5">
            ...
            {canFan && (
              <button type="button" onClick={() => toggleDay(group.date)}
                className="flex items-center gap-0.5 shrink-0 text-[10px] md:text-xs text-slate-500 hover:text-sky-500 transition-colors">
                {dayExpanded ? <><ChevronUp className="w-3 h-3 md:w-3.5 md:h-3.5" />收起</> : <><ChevronDown className="w-3 h-3 md:w-3.5 md:h-3.5" />展开全部</>}
              </button>
            )}
          </div>

          <div className={isStacked ? "relative" : "space-y-3 md:space-y-4"} style={isStacked ? { minHeight: 100 + (group.moments.length - 1) * 18 } : undefined}>
            {group.moments.map((moment, i) => {
              ...
              return (
                <motion.div key={moment.id} layout
                  ...
                  animate={{ opacity: 1, y: 0, rotate: isStacked && !isExpanded ? rot : 0, x: isStacked && !isExpanded ? offsetX : 0 }}
                  transition={{ type: "spring", stiffness: 300, damping: 25, delay: i * 0.05 }}
                  whileHover={isStacked && !isExpanded ? { rotate: 0, x: 0, y: -4, scale: 1.01 } : undefined}
                  onClick={() => setExpandedId(isExpanded ? null : moment.id)}
                  className={`${isStacked ? "absolute left-0 right-0" : "relative"} cursor-pointer`}
                  style={{ zIndex: isExpanded ? 50 : group.moments.length - i, ...(isStacked ? { top: i * 18 } : {}) }}>
